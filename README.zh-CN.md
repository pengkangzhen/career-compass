...
> 投简历**之前**的职业生涯决策引擎。在**用户画像 × 产业结构**两维基础信息上，叠加**趋势信号（顺风/逆风）与竞争强度**作为评价指标，给出**排序后的方向选项 + 依据**——回答「选什么行业、找什么岗位、凭什么进、值不值得现在进」。**到此为止**，简历优化、投递策略、面试辅导留给下游加速器工具。
...
| **宏观视角缺失** | 只对在招 JD——产业结构缺位       | **宏观视角**：用户画像 × 产业结构为基础；趋势/竞争叠加为评价指标（深/浅价值链、内卷密度进 `composite`） |
...
- 📐 **Pareto 前沿视图** —— 机会矩阵的 6 个评分维度（比较优势 / Ikigai / 顺风 / 试错 / 资格 / 竞争）本质不可公度；当前用字母档 A–F 压成单一排名会掩盖 trade-off（实测会出现「综合 A 但被严格支配」的方向）。Pareto 前沿呈现「哪些方向互不可比、需要你做价值判断」，配合两两对比 + 偏好敏感度（「如果我最在意试错成本，谁最优」）。已有原型 `career-compass pareto`，可视化形态待定（静态图信息量有限，倾向 GUI 交互式探索）
