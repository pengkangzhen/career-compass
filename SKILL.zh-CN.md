---
name: career-compass
description: 北斗星 — 职业生涯决策引擎。画像×产业趋势×匹配引擎→机会矩阵与求职执行闭环。用户要职业规划/选行业选岗位/投递反馈时使用。**Intake：编码助手对话 或 GUI 对话**，均写 data/+CLI。流程：intake→scan→analyze→execute(track/replan)→可选plan/stress-test。
---
...
【机会矩阵】opportunities.yaml → opportunities.md  ★核心交付★
        ↓ （可选）
   strategy.md · execution_pack.md · track/replan
```
...
| **3b-execute** | ❌可选 | 准备投递 / 已开始投 | `render-execution` → `track add` → `track funnel` → `replan`；JD 对齐用 `jd-analyze` |
...
**Analyze 推荐**：`brief` → `match --write-draft` → playbook 3 审阅 → `render-opportunities`（★到此为核心交付★）

**Execute 推荐**（可选）：用户选定方向并准备投递 → `render-execution` → 投递后用 `track` / `replan`
