...
| 字段 | 类型 | 说明 |
|------|------|------|
| `synergizes_with` | list[str] | 与另一层（通常主业）方向的协同关系 |
...
| 字段 | 说明 |
|------|------|
| `unified_theme` | 主业与副业如何共用能力栈、互相喂养 |
| `shared_assets` | 两层共用的资产（项目、技能、domain） |
| `synergy_notes` | 协同方式补充说明 |
| `primary` | 主业方向列表 |
| `side` | 副业方向列表 |
...
- `validate_constraints()` — geo / family
